# -*- coding: utf-8 -*-
"""
Created on Thu Jul 29 13:03:26 2021
...
class Ui_MainWindow(object):
    ...
    def DataTunggal(self):
        self.inputData = self.lineEditDataTunggal.text()
        
        self.data = {'Text':[self.inputData]}
        self.data = pd.DataFrame(self.data)

        self.data['remove_user'] = np.vectorize(pl.remove_pattern)(self.data, "@[\w]*")
        # df = df['remove_user']

        # remove angka
        self.data['remove_http'] = self.data['remove_user'].apply(lambda x: pl.remove(x))
            
        emoticons = pl.getEmoticons(self.data)
        stopwords_indonesia = pl.getStopwords(self.data)
        stemmer = pl.getStemming(self.data)
            
        #cleaning
        self.data['tweet_clean'] = self.data['remove_http'].apply(lambda x: pl.cleanTweets(x,self.data))
            
            # punct
            # remove_punct = pl.removePunct(self.df)
        self.data['Tweet'] = self.data['tweet_clean'].apply(lambda x: pl.removePunct(x))
        self.data = pd.DataFrame(self.data['Tweet'])
